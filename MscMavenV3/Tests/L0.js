...
describe("Maven L0 Suite", function () {
    before(() => {
        ...
        TestUtils_1.createTemporaryFolders();
    });
    after(() => {
        TestUtils_1.cleanTemporaryFolders();
    });
    it("run maven with all default inputs and M2_HOME not set", function (done) {
        ...
        const testRunner = new mock_test_1.MockTestRunner(testPath);
        testRunner.run();
        ...
        assert(!testRunner.stdOutContained("##vso[artifact.upload artifactname=Code Analysis Results;]"), "should not have uploaded a Code Analysis Report build summary");
        done();
    });
    ...
    it("run maven with feed", function (done) {
        ...
        const testRunner = new mock_test_1.MockTestRunner(testPath);
        testRunner.run();
        const tempDirectory = TestUtils_1.getTempDir();
    ...
    it("run maven with feed settings and spaces", function (done) {
        ...
        const testRunner = new mock_test_1.MockTestRunner(testPath);
        testRunner.run();
        const settingsPath = path.join(TestUtils_1.getTempDir(), "settings.xml");
